...
class ModelRouter {
  ...
  route(intent, options = {}) {
    ...
    if (providerMeta && credential.apiKey) {
      if (intent === 'task' && !providerMeta.supportsTools) {
        return this._fallback(intent, config.provider, { requireTools: true });
      }
      return this._buildRoute(scene, config, credential);
    }

    return this._fallback(intent, config.provider, { requireTools: intent === 'task' });
  }
  ...
  _fallback(intent, failedProvider, options = {}) {
    ...
    return this._buildGatewayRoute(scene);
  }
  ...
  registerIPCHandlers(ipcMain) {
    ipcMain.handle('modelRouter:getRoute', (event, intent, sceneConfig) => {
      ...
      return {
        ...
        scene: result.scene,
        hasKey: !!result.apiKey || result.credentialSource === 'gateway',
